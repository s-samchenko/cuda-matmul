...
// Block tile covers BMxBN of C
// Each thread computes TM×TN outputs
// Thread block: (BN/TN, BM/TM) = (16, 16) = 256 threads
...
__global__ void matmul_blocked_kernel(const float* __restrict__ A,
...
void matmul_blocked_launch(const float* A, const float* B, float* C, int N) {
    ...
    matmul_blocked_kernel<<<grid, block>>>(A, B, C, N);
}
